...
// 1. Initialize the client ONCE at the top
const client = contentful.createClient({
  accessToken: process.env.CF_CMA_TOKEN
});

// 2. Use 'export const' for the handler
export const handler = async (event, context) => {
  const origin = process.env.ALLOWED_ORIGIN || '*';
  const cors = {
    'Access-Control-Allow-Origin': origin,
  ...
  try {
    ...
    const { action, firstName, lastName, email, phone, userId, passwordHash } = body;

    if (action === 'register') {
      // Create the entry using the SDK (this is the reliable way)
      const entry = await client.createEntry('aurum', {
        fields: {
          userId: { 'en-US': userId },
          firstName: { 'en-US': firstName.trim() },
      ...
      return {
        statusCode: 201,
        headers: cors,
        body: JSON.stringify({ ok: true, entryId: publishedEntry.sys.id })
      };
    }
